...
class DataImport():
    ...
    定义mysql相关参数
    """
    DATABASE_LIST = ['CustomerDB_yh_buyer3','CustomerDB_yh_buyer2','CloudDB_yh']
    __MYSQL_BIN = r'/usr/bin'
    __MYSQL_HOST = 'localhost'
    __MYSQL_PORT = '3306'
    __MYSQL_USERNAME = 'root'
    __MYSQL_PASS = 'romens@2015'
    ...
    __HOLE_DIR = r'/home/yh/mysql/full'
    __INCREMENT_DIR = r'/home/yh/mysql/increment'
    ...
    def dataImport(self,dbname,timepoint,isfull):
        if isfull == 1:
            ...
            os.popen(self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + DataSqlPath)
    """
    导入表结构
    """
    def tableSchemaImport(self,dbname,timepoint,isfull):
        if isfull == 1:
            ...
            os.popen(self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + SchemaSqlPath)
            print self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + SchemaSqlPath
    """
    导入函数和存储过程
    """
    def funcImport(self,dbname,timepoint,isfull):
        if isfull == 1:
            ...
            os.popen(self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + FuncSqlPath)
            print self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + FuncSqlPath
    """
    导入事件
    """
    def eventsImport(self,dbname,timepoint,isfull):
        if isfull == 1:
            ...
            os.popen(self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + EventSqlPath)
            print self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + EventSqlPath
    """
    导入触发器
    """
    def triggersImport(self,dbname,timepoint,isfull):
        if isfull == 1:
            ...
            os.popen(self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + triggersSqlPath)
            print self.__IMPORT_COMMAND + self.__LOGINARGS + ' ' + dbname + '<' + triggersSqlPath
    """
    恢复前先删除旧的数据库，重新新建
    """
