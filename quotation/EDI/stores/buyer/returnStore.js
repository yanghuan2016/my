...
var returnStore = assign({}, EventEmitter.prototype, {
    ...
    getReturnDetail: function(returnID){
        ...
        //    returnDetailList = returnList[_.findIndex(returnList, {"billNo":returnID})];
        //}
        if(returnDetailList){
...
Dispatcher.register(function(action){
    switch (action.type) {
        case actionConstants.RETURN_LIST:
            logger.trace(action);
            returnList = action.data;
            break;
        case actionConstants.RETURN_DETAIL:
            logger.trace(action);
            returnDetailList = action.data;
            break;
        case actionConstants.RETURN_BATCH:
            logger.trace(action);
            returnBatch = returnDetailList.returnDetails[_.findIndex(returnDetailList.returnDetails, {"guid":action.data})];
