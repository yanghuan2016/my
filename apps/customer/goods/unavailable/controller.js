...
function disabledController(app) {
    logger.enter();
    ...
    app.get(APPURL + "/list", auth.restrict,
        ...
        getGoodsListHandler);
    app.post(APPURL + "/enableGoods", auth.restrict, auth.acl(__FEATUREENUM.FP_INVENTORY_GOODS), postEnableGoodsHandler);
}
...
function getGoodsListHandler(req, res, next) {
    ...
    dataService.commonData(req, function (data) {
        model.getGoodsList(customerDB, paginator, goodTypeId, function (err, goodsList) {
            if (err) {
                logger.error(err);
                return next();
            }
            ...
            data.handlerType = 'unavailable';


            res.render('customer/center/goods/unavailable', {data: data});
        });
    });
}

function postEnableGoodsHandler(req, res) {
    logger.enter();
    var goodsIds = req.body.goodsIds;
    var customerDB = req.session.customer.customerDB;
    model.postGoodsEnable(customerDB, goodsIds,function(error,result){
        if (error) {
            logger.sqlerr(error);
            return res.json(new FeedBack(FBCode.DBFAILURE, "数据库开小差了,请稍后再试." + error.code));
        }
        res.json(new FeedBack(FBCode.SUCCESS, "上架成功", {goodsIds: goodsIds, result: result}));
    });
