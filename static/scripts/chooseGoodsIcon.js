...
$(document).delegate('.addThisToSC', 'click', function () {
    ...
    if(localStorage.currentICONShowCaseGoodsIds){
        ...
        var currentIds=localStorage.currentICONShowCaseGoodsIds;
        var currentArray=currentIds.split(',');

        if(currentArray.length==8){
            parent.window.artDialogAlertModal("橱窗最多8个商品,请先删掉一些吧.");
            return;
        }
    ...
    insertStr+='<a  href="javascript:void(0)" dom-add="true"  data-id="'+currentGoodsId+'" class="deleteSCGoodBtn">'+
        '<i class="fa fa-remove tooltipped" data-toggle="tooltip"  data-placement="left" data-sctype="scICON" title="删除"  style="margin-right: 2px"></i></a>';
    ...
    if(localStorage.currentICONShowCaseGoodsIds){
        currentArray=localStorage.currentICONShowCaseGoodsIds.split(',');
        currentArray.push(currentGoodsId);
        localStorage.currentICONShowCaseGoodsIds=currentArray;
    }
    else{
        var arry=[];
        arry.push(currentGoodsId);
        localStorage.currentICONShowCaseGoodsIds=arry;
    }
